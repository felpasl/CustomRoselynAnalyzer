...
This rule warns when infrastructure-layer services (such as database access, HTTP calls, or file I/O) are invoked from within loop bodies or lambda-based iteration constructs like `List<T>.ForEach`. Infrastructure operations are typically costly and should be performed outside of iterations to avoid performance issues.
...
public class OrderProcessor
{
    ...
    {
        // CR0003 violation: Infrastructure call inside lambda iteration
        orders.ForEach(order => _repository.Save(order));
    }
}
```
...
The analyzer identifies infrastructure by checking if the called method belongs to a namespace containing "Infrastructure" (case-insensitive). It also examines lambda expressions passed to well-known iteration helpers (currently `ForEach`) so the same protections apply to method-based loops.
